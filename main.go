...
func main() {
	s := "\tpublic static @property Color "
	sa := "() { return new Color(0x"
	se := "); }\n\n"

	oister := "\t/*\n\t\tCOLORS AUTOGENERATED WITH COLORGEN SCRIPT\n\t*/\n"
	for k, i := range colors {
		colorun := strings.Replace(i, " ", "", -1)
		colorun = strings.Replace(colorun, "'", "", -1)
		oister += "\t/// Color named " + i + ", with hex: " + k + "\n" + s + colorun + sa + k + "FF" + se
	}
	oister += "\n\t/*\n\t\tEND OF AUTO GENERATED COLORS\n\t*/\n"
	err := ioutil.WriteFile("colors.d", []byte(oister), 0644)
	if err != nil {
		println(err)
	}
}
